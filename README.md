# PaliGemma Fine-tuning on Flickr8k Dataset
...
# 5. Saves the trained model to /kaggle/working/PaliGemma_Fine_Tune_Flickr8k/ but you can update it to your desired location if you are running locally.
```

## Inference

The inference scripts (`paste-2.txt` and `PaliGemma_test_script.py`) provide:
...
MODEL_PATH = "/kaggle/working/PaliGemma_Fine_Tune_Flickr8k/paligemma_flickr8k.params.f16.npz" update the location to the actual path you provided to be stored in if you are running locally
...
1. **Data Loading**: Flickr8k images and captions are loaded and preprocessed out of which 1000 images are used due to system constraints you can use full 8000 and increased number of epochs for better results.
...
2. **Loading Errors**: Ensure proper model path and file permissions
3. **Dataset Issues**: Verify Flickr8k dataset structure and file paths
4. **JAX Issues**: Ensure proper JAX installation and device configuration
